...
const Home = () => {
  ...
  const handleInvite = async (listing) => {
    ...
    if (!user.profile_completed || !user.profile?.institution || !user.profile?.role || !user.profile?.current_province) {
      toast.error('Talep göndermek için önce profilinizi tamamlamanız gerekiyor.');
      navigate('/dashboard');
      return;
    }
    try {
      await api.post('/invitations', { listing_id: listing.id });
      toast.success('Talep başarıyla gönderildi.');
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground">Talep kabul edilene kadar iletişim bilgileriniz gizli kalır.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground">Kabul edilen talepler sonrası direkt mesajlaşma imkanı.</p>
            </div>
          </div>
        </div>
      </div>

...
const Dashboard = () => {
  ...
  const handleCreateSupportTicket = async (e) => {
    ...
    try {
      const response = await api.post('/support-tickets', supportFormData);
      setSupportTickets([response.data.ticket, ...supportTickets]);
          ...
          <TabsContent value="support" className="space-y-6">
            ...
            {supportTickets.length === 0 ? (
              <Card className="p-12 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-2">Sorularınız veya önerileriniz için yeni bir talep oluşturabilirsiniz.</p>
              </Card>
            ) : (
              <div className="space-y-4">
      ...
      <Dialog open={showSupportDialog} onOpenChange={setShowSupportDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Sorularınız, önerileriniz veya şikayetleriniz için bize yazın.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreateSupportTicket} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="ticket-message">Mesaj * <span className="text-muted-foreground font-normal">({supportFormData.message.length}/1000)</span></Label>
              <Textarea
                id="ticket-message"
            ...
            <DialogFooter>
              ...
              <Button 
                type="submit" 
                disabled={submittingTicket || !supportFormData.subject.trim() || !supportFormData.message.trim()}
                className="bg-gradient-to-r from-amber-500 to-orange-500"
                data-testid="submit-ticket-btn"
              >
                {submittingTicket ? 'Gönderiliyor...' : 'Gönder'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
